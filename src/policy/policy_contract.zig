...
//!
//! Blueprint principle: "Policy MUST NOT execute" Î“Ã‡Ã¶ it only sets
//! PolicyAction on the event. "Enforcement MUST NOT decide" Î“Ã‡Ã¶ it
//! only executes what policy decided.
//!
//! Contract flow:
//!   1. Detection produces DetectionResult
//!   2. Policy Engine evaluates result + context Î“Ã¥Ã† sets policy_action
//!   3. PEP reads policy_action Î“Ã¥Ã† executes enforcement
...
// ============================================================
// Policy Rule (maps detection verdict Î“Ã¥Ã† policy decision)
// ============================================================
...
// ============================================================
// Evaluates detection results + context Î“Ã¥Ã† produces PolicyDecision
...
pub const PolicyEngine = struct {
    ...
    pub fn evaluate(self: *PolicyEngine, result: detection.DetectionResult, ctx: PolicyContext) PolicyDecision {
        ...
        // No rule matched Î“Ã‡Ã¶ return default
...
pub const EnforcementResult = enum(u8) {
    ...
    skipped = 3,        // Action was allow/log_only Î“Ã‡Ã¶ no enforcement needed
};
...
pub const PEP = struct {
    ...
    pub fn enforce(self: *PEP, decision: PolicyDecision, event: *canonical.CanonicalEvent) EnforcementResult {
        ...
        switch (decision) {
            ...
            .block => {
                // AEGIS-011: IPS inline blocking Î“Ã‡Ã¶ actually call WFP IOCTL
                // through the Rust PEP path (T11: single PEP path to enforcement)
                const pep = @import("../core/rust_pep.zig");
                ...
                // No source IP Î“Ã‡Ã¶ can't block (host event)
                _ = self.total_enforced.fetchAdd(1, .monotonic);
