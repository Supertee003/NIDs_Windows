...
// HRESULT_FROM_WIN32(ERROR_IO_PENDING) = 0x800703E7 (minifilter-specific)
...
pub fn minifilterReaderLoop() void {
    ...
    std.log.info("[MINI] Thread 4 started - connecting to minifilter driver", .{});
    ...
    while (true) {
        ...
        if (hr >= 0) {
            ...
            std.log.info("[MINI] Connected to AegisMinifilterPort", .{});
            break;
        }

        retry_count += 1;
        if (retry_count == 1) {
            std.log.warn("[MINI] Driver not available (hr=0x{x}), retrying every 10s", .{@as(u32, @bitCast(hr))});
    ...
    while (true) {
        ...
        if (hr >= 0) {
            // Completed synchronously Î“Ã‡Ã¶ process immediately
            g_events_received += 1;
            ...
            if (hr_u32 == HRESULT_PENDING) {
                // I/O is pending Î“Ã‡Ã¶ wait with 1s timeout via shared helper (Phase 9)
                const wait_result = win32_io.waitOverlapped(hPort, &overlapped, io_event, IO_POLL_TIMEOUT_MS);
                switch (wait_result) {
                    .timeout => continue,
                    .wait_error => {
                        std.log.warn("[MINI] WaitForSingleObject failed", .{});
                        std.time.sleep(100 * std.time.ns_per_ms);
                        continue;
                    },
                    .result_error => continue,
                    .completed, .completed_after_wait => {
                        // Event signaled Î“Ã‡Ã¶ message available, process it
                        var bytes_xfer: u32 = 0;
                        if (win32_io.GetOverlappedResult(hPort, &overlapped, &bytes_xfer, 0) != 0) {
                            g_events_received += 1;
                            // Parse the AEGIS_FILE_EVENT header
                            if (msg_buf.len >= @sizeOf(AegisFileEvent)) {
                                const evt: *align(1) const AegisFileEvent =
                                ...
                                // (was: ch >= 0x20 and ch < 0x7F Î“Ã‡Ã¶ dropped CJK/emoji paths silently)
                                var name_buf: [512]u8 = undefined;
                                var name_len_out: usize = 0;
                                for (file_name) |ch| {
                                    if (ch >= 0x20 and name_len_out < name_buf.len) {
                                        // Pass through printable bytes (includes UTF-8 multi-byte sequences
                                        // from the kernel's WCHARÎ“Ã¥Ã†CHAR conversion)
                                        name_buf[name_len_out] = ch;
                                        name_len_out += 1;
                                    } else if (ch == 0) {
                                        // Null terminator Î“Ã‡Ã¶ stop
                                        break;
                                    }
                                }
                                const name_str = name_buf[0..name_len_out];
