//! nose_contract.zig - AEGIS Nose Î“Ã¥Ã† Event Fabric Contract (Phase 25, AEGIS-006)
//!
//! Defines the contract between Sensors (Nose) and the Event Fabric (Queue).
//! Sensors MUST use this interface to submit events Î“Ã‡Ã¶ they MUST NOT
...
//!   3. Sensor calls nose_submitEvent() Î“Ã‡Ã¶ this validates and enqueues
...
const wire = @import("../contract/wire_event.zig");
const pq = @import("../core/priority_queue.zig");
...
pub const SubmitResult = enum {
    ...
    dropped_at_source,  // Event dropped at sensor before reaching fabric
    dropped_by_fabric,   // Event dropped by fabric (queue full / priority overflow)
...
test "submitEvent drops on overflow" {
    ...
    try std.testing.expect(submitEvent(event) == .accepted);
    try std.testing.expect(submitEvent(event) == .dropped_by_fabric); // Queue full
}
