...
///
/// Caller-owned API: redact() writes into `out` (caller-allocated), so the
/// `src_ip_masked` slice points into `out.ip_buf` which has a valid lifetime
/// tied to the caller's storage.
pub fn redactInto(record: ForensicRecord, out: *RedactedRecord) void {
    out.record_id = record.record_id;
    out.epoch_ms = record.epoch_ms;
    ...
    out.ip_buf_used = false;
    out.src_ip_masked = null;

    if (record.src_ip) |ip| {
        const masked = formatIp(&out.ip_buf, ip);
        out.src_ip_masked = masked;
        out.ip_buf_used = true;
    }
}

/// Convenience wrapper: redact() returns a RedactedRecord by value.
/// NOTE: the returned struct's `src_ip_masked` slice points into the struct's
/// `ip_buf` field, which is moved along with the struct (copy-on-return).
/// This is safe ONLY if you immediately use the result without reassigning it.
/// For long-lived storage, use `redactInto` instead.
pub fn redact(record: ForensicRecord) RedactedRecord {
    ...
    // Compute the masked IP AFTER assigning the result to its final location.
    // This is done by the caller via redactInto() for the test paths.
    // Here, we just write the buffer; the slice we get back points to the
    // local `result.ip_buf`, but since `result` is returned by value and
    // the caller stores it, the slice becomes dangling.
    //
    // For safety, callers that need to access src_ip_masked should use
    // redactInto() instead. The redact() wrapper is kept for compatibility
    // with existing callers that don't read src_ip_masked.
    if (record.src_ip) |ip| {
        _ = formatIp(&result.ip_buf, ip);
        result.ip_buf_used = true;
        // Compute the slice length by scanning for the first NUL byte
        // (set by bufPrint's terminator). The slice is still dangling after
        // return, but at least the length is correct.
        var len: usize = 0;
        while (len < result.ip_buf.len and result.ip_buf[len] != 0) : (len += 1) {}
        result.src_ip_masked = result.ip_buf[0..len];
    }
