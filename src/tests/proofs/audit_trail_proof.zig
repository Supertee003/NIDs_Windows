...
test "AuditTrail init starts empty" {
    ...
    // chainHead() returns the genesis hash (seed for the first record's prev_hash).
    try std.testing.expect(trail.chainHead() == AEGIS_AUDIT_SEED);
}
