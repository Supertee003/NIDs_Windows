...
pub const MAX_PROM_LABELS: usize = 4;

pub const PromMetric = struct {
    name: []const u8,
    help: []const u8,
    metric_type: PromMetricType,
    /// Inline array of labels (avoids dangling slice after function return).
    labels: [MAX_PROM_LABELS]PromLabel,
    /// Number of valid labels in the inline array.
    label_count: usize,
    value: f64,
};
...
pub fn toPromMetrics(event: TelemetryEvent, out: []PromMetric) usize {
    ...
    if (count < out.len) {
        var labels: [MAX_PROM_LABELS]PromLabel = .{
            .{ .key = "", .value = "" },
            .{ .key = "", .value = "" },
            .{ .key = "", .value = "" },
            .{ .key = "", .value = "" },
    ...
    if (count < out.len) {
        ...
        labels[0] = .{ .key = "verdict", .value = event.verdict };
    ...
    if (count < out.len) {
        ...
        labels[0] = .{ .key = "severity", .value = event.severity };
...
pub fn verifyPrometheus() PrometheusCheck {
    ...
    const metric_has_labels = metrics[0].label_count >= 1 and
...
test "toPromMetrics counter has action label" {
    ...
    try std.testing.expect(metrics[0].label_count >= 1);
