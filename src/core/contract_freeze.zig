...
const wire_event = @import("../contract/wire_event.zig");
...
pub fn verifyWireABI() WireABIStatus {
    // Check that wire_event module has required functions
    const has_serialize = @hasDecl(wire_event, "serializeEvent");
    const has_deserialize = @hasDecl(wire_event, "deserializeEvent");
    const has_is_valid = @hasDecl(wire_event, "isValidFrame");

    if (!has_serialize) {
        return .{ .status = .violation, .reason = "wire_event.serializeEvent missing" };
    }
    if (!has_deserialize) {
        return .{ .status = .violation, .reason = "wire_event.deserializeEvent missing" };
    }
    if (!has_is_valid) {
        return .{ .status = .violation, .reason = "wire_event.isValidFrame missing" };
    }
