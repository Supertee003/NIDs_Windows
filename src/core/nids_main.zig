...
const windows_capture = @import("../capture/windows_capture.zig");
...
pub fn main() !void {
    ...
    // delegate probe per COMPONENT_MATRIX.md â”¬Âº2.1).
    //
    ...
    // probe core health without spawning a child process (contract â”¬Âº4).
...
// Implements \\.\pipe\aegis-core-health (RUNTIME_CONTRACT.md â”¬Âº4).
...
fn healthServerLoop() void {
           ...
           !bridge_init.g_shutdown.load(.seq_cst))
    {
        ...
        // signal Î“Ã‡Ã¶ we must skip the wait and serve immediately.
        var serve_now = false;
        if (connected == 0) {
        ...
        if (!serve_now) {
            ...
            if (wait_result == WaitTimeout) {
                ...
                // Not shutting down yet Î“Ã‡Ã¶ close this pipe instance and recreate.
                _ = health.CancelIoEx(hp, @ptrCast(&overlapped));
                _ = health.CloseHandle(hp);
                _ = health.CloseHandle(h_event);
                continue;
            ...
            if (wait_result != WaitObject0) {
                // Some other error Î“Ã‡Ã¶ clean up and try again.
                _ = health.CloseHandle(hp);
                _ = health.CloseHandle(h_event);
                continue;
            }
