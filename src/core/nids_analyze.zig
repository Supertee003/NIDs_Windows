...
// =================================================================
// [ BP-O1/O2: OVERLAPPED I/O Î“Ã‡Ã¶ shutdown-responsive blocking calls ]
// =================================================================
...
// Phase 9: Consolidated declarations into shared win32_io.zig module.
...
// =================================================================
// CRC32 has a 32-bit collision space Î“Ã‡Ã¶ trivially constructible by an attacker
...
// P-09: Global pipe bytes counter (prevents RAM exhaustion via 100 sessions â”œÃ¹ 512MB)
...
//         Old behavior: catch -> return; (success) Î“Ã¥Ã† caller thinks rules loaded
//         New behavior: catch -> log + return err Î“Ã¥Ã† caller knows rules failed
pub fn reload_rules_atomic(allocator: std.mem.Allocator) !void {
    ...
    if (std.fs.cwd().openFile("Rules.json.hmac", .{})) |hmac_file| {
        ...
        if (hmac_len > 0) {
            ...
            // G29 fix: std.process.getenv was removed in Zig 0.13.0.
            // Use std.process.getEnvVarOwned which takes an allocator
            // and returns an Owned slice (must be freed).
            const hmac_key_owned = std.process.getEnvVarOwned(allocator, "AEGIS_RULES_HMAC_KEY") catch null;
            defer if (hmac_key_owned) |owned| allocator.free(owned);
            const hmac_key: []const u8 = hmac_key_owned orelse "AEGIS_NIDS_INTEGRITY_KEY_v1";
        ...
        // No HMAC file Î“Ã‡Ã¶ skip verification (first run or not configured)
    }
...
fn pushTier1Match(
    ...
    _ = g_bridge_ipc_pushes.fetchAdd(1, .monotonic);
...
/// Called when data.len > ANALYZE_MAX_PAYLOAD Î“Ã‡Ã¶ caller has already truncated.
fn inspect_packet_oversized(data: []const u8, ctx: PacketContext) !bool {
    ...
    if (final_matched_rule) |rule| {
        if (matched_rule_idx < current_ruleset.match_counts.len) {
            _ = current_ruleset.match_counts[matched_rule_idx].fetchAdd(1, .monotonic);
        }
        _ = g_total_matches.fetchAdd(1, .monotonic);
        const alert = .{
...
pub fn inspect_packet(data: []const u8, ctx: PacketContext) !bool {
    ...
    // (was: return true Î“Ã‡Ã¶ attacker could hide payloads beyond 65536 bytes)
    if (data.len > ANALYZE_MAX_PAYLOAD) {
        ...
        _ = g_oversized_read.fetchAdd(1, .monotonic);
    ...
    if (final_matched_rule) |rule| {
        ...
        _ = g_total_forwarded.fetchAdd(1, .monotonic);
...
fn handle_pipe_client(hPipe: win.HANDLE, allocator: std.mem.Allocator) void {
    ...
    while (true) {
        ...
        _ = g_pipe_global_bytes.fetchAdd(bytes_read, .monotonic);
        // BP179: Track total bytes processed (pipe path)
        _ = g_total_bytes.fetchAdd(bytes_read, .monotonic);
        ...
        const is_safe = inspect_packet(buf[0..bytes_read], ctx) catch |analyze_err| blk: {
            ...
            _ = g_analyze_errors.fetchAdd(1, .monotonic);
            break :blk true;
        };
...
fn pipe_listener(allocator: std.mem.Allocator) !void {
    ...
    while (!g_shutdown_requested.load(.acquire)) {
        ...
        if (!connected and @intFromEnum(err) == ERROR_IO_PENDING) {
            // BP-O2: Wait for client with 1s timeout via shared helper
            const wait_result = win32_io.waitOverlapped(hPipe, &overlapped, io_event, IO_POLL_TIMEOUT_MS);
            switch (wait_result) {
                .timeout => {
                    // No client yet Î“Ã‡Ã¶ check shutdown and retry ConnectNamedPipe
                    _ = win.CloseHandle(hPipe);
                    continue;
                },
                .wait_error => {
                    std.log.warn("[PIPE] WaitForSingleObject failed", .{});
                    _ = win.CloseHandle(hPipe);
...

        {
            ...
            // BP205: Use timedWait(0) to prevent blocking during shutdown with full semaphore
            if (connection_semaphore.timedWait(0)) |_| {} else |_| {
                std.log.warn("[SEMAPHORE] Pipe: connection limit reached ({d}), rejecting client", .{MAX_CONCURRENT_CONNECTIONS});
                _ = DisconnectNamedPipe(hPipe);
                win.CloseHandle(hPipe);
                _ = g_analyze_errors.fetchAdd(1, .monotonic);
                _ = g_rejected_connections.fetchAdd(1, .monotonic);
                continue;
            }
            // BP189: Relaxed for pure counter (ordering not needed for increment)
            _ = g_total_connections.fetchAdd(1, .monotonic);
            // BP180: Track pipe connections
            _ = g_pipe_connections.fetchAdd(1, .monotonic);
...
fn handle_tcp_client(stream: net.Stream, remote_addr: net.Address, allocator: std.mem.Allocator) void {
    ...
    while (true) {
        ...
        _bp94_last_act_ns = @intCast(std.time.nanoTimestamp());
        if (len == 0) {
        ...
        // BP92: TTFB tracking
        if (_bp92_ttfb_ns == 0) {
            _bp92_ttfb_ns = @intCast(std.time.nanoTimestamp());
        ...
        _ = g_total_bytes.fetchAdd(len, .monotonic);
        // BP166: Per-client packet rate limiting
        pkt_rate_count += 1;
        {
            const _now_ns: i64 = @intCast(std.time.nanoTimestamp());
...
fn tcp_listener(allocator: std.mem.Allocator) !void {
    ...
    // G29 fix: Zig 0.13.0's ListenOptions does not have a `.backlog` field
    // (was removed in 0.13.0; the kernel default is now used). We keep
    // TCP_LISTEN_BACKLOG as a documented constant for operator reference
    // but no longer pass it to listen().
    ...
    while (!g_shutdown_requested.load(.acquire)) {
        ...
        var poll_fds = [_]WSAPollfd{
            .{ .fd = @intFromPtr(server.stream.handle), .events = POLLIN, .revents = 0 },
        };
        const poll_ret = WSAPoll(&poll_fds, 1, 1000);
        if (poll_ret == 0) continue; // timeout Î“Ã‡Ã¶ check shutdown flag
        if (poll_ret < 0) {
            std.log.warn("[TCP] WSAPoll error, retrying", .{});
            std.time.sleep(100 * std.time.ns_per_ms);
            continue;
        }
        // Socket is readable Î“Ã‡Ã¶ accept() won't block
...
// =================================================================
// [ P-11: RULES WATCHDOG Î“Ã‡Ã¶ hot-reload on Rules.json mtime change ]
// =================================================================
...
fn rulesWatchdog(allocator: std.mem.Allocator) void {
    ...
    while (true) {
        ...
        if (std.fs.cwd().statFile("Rules.json")) |stat| {
            ...
            // File deleted Î“Ã‡Ã¶ keep last_mtime, will reload when recreated
        }
    }
...
/// This demonstrates the Golden Path: Sensor Î“Ã¥Ã† Queue Î“Ã¥Ã† Detection Î“Ã¥Ã† Policy.
fn eventFabricDrain() void {
    ...
    while (true) {
        ...
        if (nose.hasEvents()) {
            ...
            // No events Î“Ã‡Ã¶ sleep 100ms before checking again
            std.time.sleep(100 * std.time.ns_per_ms);
        }
    }
}
...
fn bridgeStatusReporter() void {
    while (true) {
        ...
        if (g_shutdown_requested.load(.acquire)) break;
        // IR-12: Shutdown-responsive sleep (was 30s blocking Î“Ã‡Ã¶ now polls every 1s)
        ...
        std.debug.print("  Active threads: {d}  |  Analyze errors: {d}\n", .{
            ...
            g_analyze_errors.load(.monotonic),
        });
        ...
        const _total_b: u64 = g_total_bytes.load(.monotonic);
        ...
        // BP174: Consistent relaxed ordering for error counter reads
        const errs: u64 = @as(u64, g_analyze_errors.load(.monotonic));
        ...
        std.debug.print("  Security rejections: {d}\n", .{g_rejected_connections.load(.monotonic)});
        ...
        if (rs165) |rs| {
            ...
            std.debug.print("  Rule matches: {d} (v{d})\n", .{ g_total_matches.load(.monotonic), g_ruleset_version.load(.acquire) });
    ...
    std.log.info("[SHUTDOWN] conn={d} bytes={d} errors={d} rej={d} matches={d} fwd={d} ipc={d} uptime={d}s", .{
        ...
        g_total_forwarded.load(.monotonic),
        g_bridge_ipc_pushes.load(.monotonic),
        @as(u64, @intCast(@max(@as(i128, 0), std.time.nanoTimestamp() - g_start_time_ns) / std.time.ns_per_s)),
    });
    ...
    const sh_err: u64 = @as(u64, g_analyze_errors.load(.monotonic));
    ...
    const sh_matches: u64 = g_total_matches.load(.monotonic);
    const sh_fwd: u64 = g_total_forwarded.load(.monotonic);
    ...
    const sh_bytes: u64 = g_total_bytes.load(.monotonic);
...
pub fn analyze_packets(allocator: std.mem.Allocator) void {
    ...
    g_start_time_ns = @intCast(std.time.nanoTimestamp());
    ...
    // G29 fix: std.process.getenv was removed in Zig 0.13.0; use getEnvVarOwned.
    if (std.process.getEnvVarOwned(allocator, "AEGIS_TCP_PORT")) |port_str| {
