...
rs_files = {
    "windows_sec_monitor.rs": os.path.join(PROJECT_ROOT, "mouth", "windows_sec_monitor.rs"),
    "aegis_mouth_tui.rs": os.path.join(PROJECT_ROOT, "mouth", "aegis_mouth_tui.rs"),
    "src/lib.rs": os.path.join(PROJECT_ROOT, "src", "lib.rs"),
    "Cargo.toml": os.path.join(PROJECT_ROOT, "Cargo.toml"),
}
...
# Find DLL
dll_paths = [
    os.path.join(PROJECT_ROOT, "target", "release", "sec_monitor.dll"),
    os.path.join(PROJECT_ROOT, "zig-out", "bin", "sec_monitor.dll"),
]
...
result("sec_monitor.dll found", shield_dll is not None,
       shield_dll.replace(PROJECT_ROOT + os.sep, "") if shield_dll else "not built — run: cargo build --release")
...
exe_path = os.path.join(PROJECT_ROOT, "dist/windows_sec_monitor.exe")
result("dist/windows_sec_monitor.exe pre-built", os.path.exists(exe_path),
       "found" if os.path.exists(exe_path) else "not built — use: rustc -O windows_sec_monitor.rs -o dist/windows_sec_monitor.exe")

# Check if Mouth is running
mouth_running = False
mouth_pid = None
try:
    r = subprocess.run(["tasklist", "/FI", "IMAGENAME eq dist/windows_sec_monitor.exe", "/NH"],
