//! lifecycle.zig - AEGIS Runtime Lifecycle (Rewrite Phase 9)
//!
//! Manages init/shutdown of all subsystems in correct order.
//! main() calls runtime.start() and runtime.shutdown() - nothing else.
//!
//! Phase 9 changes (on top of Phase 8):
//!   - Added Correlation Engine to start()/shutdown() sequence.
//!   - Correlation is initialized AFTER Aggregator (it consumes AggregatedVerdict).
...
test "lifecycle: flow_int, detection_int, aggregator, and correlation initialized after start" {
    if (fabric.isInitialized()) {
        shutdown();
    }
