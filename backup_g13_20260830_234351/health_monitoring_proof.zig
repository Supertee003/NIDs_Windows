...
pub const MetricsSnapshot = struct {
    ...
    pub fn p50LatencyUs(self: MetricsSnapshot) u64 {
        ...
        return percentile(self.latency_samples_us[0..self.latency_sample_count], 50);
    }

    /// Calculate p95 latency in microseconds.
    pub fn p95LatencyUs(self: MetricsSnapshot) u64 {
        if (self.latency_sample_count == 0) return 0;
        return percentile(self.latency_samples_us[0..self.latency_sample_count], 95);
    }

    /// Calculate p99 latency in microseconds.
    pub fn p99LatencyUs(self: MetricsSnapshot) u64 {
        if (self.latency_sample_count == 0) return 0;
        return percentile(self.latency_samples_us[0..self.latency_sample_count], 99);
    }
...
/// Samples are sorted in-place (mutates the input slice).
fn percentile(samples: []u64, p: u8) u64 {
    if (samples.len == 0) return 0;
    if (samples.len == 1) return samples[0];

    // Sort the samples in-place (using insertion sort for small arrays).
...
test "MetricsSnapshot.percentiles" {
    ...
    // Note: percentile() sorts in place, so we need to call p50/p95/p99 on a fresh snapshot.
    // For testing, we verify the percentile function directly with a sorted copy.
    var sorted_samples = [_]u64{ 100, 200, 300, 400, 500 };
    const p50 = percentile(&sorted_samples, 50);
    var sorted_samples2 = [_]u64{ 100, 200, 300, 400, 500 };
    const p95 = percentile(&sorted_samples2, 95);
    var sorted_samples3 = [_]u64{ 100, 200, 300, 400, 500 };
    const p99 = percentile(&sorted_samples3, 99);
