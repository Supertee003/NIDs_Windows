//! runtime/dispatcher.zig - AEGIS Runtime Dispatcher (Rewrite Phase 10)
//!
...
pub fn processEvent(event: canonical.CanonicalEvent) void {
        ...
        if (evidence_list.count > 0) {
            ...
            if (g_aggregator) |*agg| {
                ...
                if (correlation_int.isInitialized()) {
                    const alerts = correlation_int.processVerdict(event, flow_update_opt, av);
                    for (alerts) |a| {
                        if (a) |alert| {
                ...
                if (threat_intel_int.isInitialized()) {
                    const ti_match = threat_intel_int.enrichEvent(event);
    ...
    // Phase 12+: policy_int.evaluateAndEnforce(event, av, alerts, ti_match)
    // Phase 14+: forensics_int.logPipelineResult(event, av, alerts, ti_match)
}
