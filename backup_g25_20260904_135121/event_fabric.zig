...
pub const FabricMetrics = struct {
    ...
    /// Phase C: Accounting invariant check.
    /// Only accepted events participate in the conservation equation.
    /// dropped/rejected events never entered the queue (separate exit paths).
    pub fn verifyInvariant(self: FabricMetrics) bool {
        const input = self.total_accepted;
        const output = self.total_popped + self.total_expired;
        // pending events are in-flight (accepted but not yet popped/expired)
        return input == output + self.pending;
    }

    /// Phase C: Returns processing latency in milliseconds.
    pub fn lastPopLatencyMs(self: FabricMetrics) f64 {
        return @as(f64, @floatFromInt(self.last_pop_latency_ns)) / 1_000_000.0;
    }

    pub fn maxPopLatencyMs(self: FabricMetrics) f64 {
        return @as(f64, @floatFromInt(self.max_pop_latency_ns)) / 1_000_000.0;
    }
};
...
// Phase C: New metrics for accounting invariant
var g_total_expired: std.atomic.Value(u64) = std.atomic.Value(u64).init(0);
var g_high_watermark: std.atomic.Value(usize) = std.atomic.Value(usize).init(0);
var g_backpressure_events: std.atomic.Value(u64) = std.atomic.Value(u64).init(0);
...
pub fn initFabric(allocator: std.mem.Allocator, config: FabricConfig) !void {
    ...
    // Phase C: reset new counters
    g_total_expired.store(0, .monotonic);
    g_high_watermark.store(0, .monotonic);
    g_backpressure_events.store(0, .monotonic);
...
pub fn submitWithBackpressure(event: canonical.CanonicalEvent) SubmitWithPressure {
    ...
    if (g_priority_queue) |*queue| {
        ...
        // Phase C: Track high watermark (max queue depth ever seen)
        const current_depth = queue.len();
        const prev_wm = g_high_watermark.load(.monotonic);
        if (current_depth > prev_wm) {
            _ = g_high_watermark.cmpxchgStrong(prev_wm, current_depth, .monotonic, .monotonic);
        }

        // Phase C: Track backpressure events (pressure went to high or saturated)
        if (pressure == .high or pressure == .saturated) {
            _ = g_backpressure_events.fetchAdd(1, .monotonic);
        }
...
pub fn getMetrics() FabricMetrics {
    ...
    return .{
        ...
        .total_expired = g_total_expired.load(.monotonic),
        .high_watermark = g_high_watermark.load(.monotonic),
        .backpressure_events = g_backpressure_events.load(.monotonic),
...
test "initFabric and shutdownFabric lifecycle" {
    if (g_fabric_initialized) shutdownFabric(std.testing.allocator);
...
// ============================================================
// Phase C: Event Fate Accounting + Queue Saturation Tests
// ============================================================

test "Phase C: accounting invariant holds for simple submit+pop" {
    if (g_fabric_initialized) shutdownFabric(std.testing.allocator);
    try initFabric(std.testing.allocator, .{ .capacity_per_priority = 16 });
    defer shutdownFabric(std.testing.allocator);
    ...
    const m = getMetrics();
    // input = accepted = 1
    // output = popped(0) + dropped(0) + rejected(0) + expired(0) = 0
    // pending = 1
    // invariant: input == output + pending
    try std.testing.expect(m.verifyInvariant());
}

test "Phase C: accounting invariant holds after pop" {
    if (g_fabric_initialized) shutdownFabric(std.testing.allocator);
    try initFabric(std.testing.allocator, .{ .capacity_per_priority = 16 });
    defer shutdownFabric(std.testing.allocator);
    ...
    const m = getMetrics();
    // input = accepted = 1
    // output = popped(1) + dropped(0) + rejected(0) + expired(0) = 1
    // pending = 0
    try std.testing.expect(m.verifyInvariant());
    try std.testing.expect(m.total_accepted == 1);
    try std.testing.expect(m.total_popped == 1);
...
test "Phase C: accounting invariant holds on overflow (fabric_dropped)" {
    if (g_fabric_initialized) shutdownFabric(std.testing.allocator);
    try initFabric(std.testing.allocator, .{ .capacity_per_priority = 4 });
    defer shutdownFabric(std.testing.allocator);

    // Submit 10 events to a capacity-4 queue (6 should be dropped)
    var i: u64 = 0;
    while (i < 10) : (i += 1) {
        const event = canonical.create(.zig_core);
        _ = submitEvent(event);
    ...
    const m = getMetrics();
    // input = accepted (at most 4 accepted, 6 dropped)
    // invariant: accepted == popped(0) + dropped + pending
    try std.testing.expect(m.verifyInvariant());
    try std.testing.expect(m.total_dropped > 0);
}

test "Phase C: accounting invariant holds with rejected events" {
    if (g_fabric_initialized) shutdownFabric(std.testing.allocator);
    try initFabric(std.testing.allocator, .{
        .capacity_per_priority = 16,
        .validate_on_submit = true,
    });
    defer shutdownFabric(std.testing.allocator);

    // Submit a valid event
    const valid = canonical.create(.zig_core);
    _ = submitEvent(valid);

    // Submit an invalid event (wrong magic)
    var invalid = canonical.create(.zig_core);
    invalid.magic = 0xDEADBEEF;
    _ = submitEvent(invalid);
...
test "Phase C: producer flood test (1000 events, capacity 16)" {
    if (g_fabric_initialized) shutdownFabric(std.testing.allocator);
    try initFabric(std.testing.allocator, .{ .capacity_per_priority = 16 });
    defer shutdownFabric(std.testing.allocator);

    // Flood: submit 1000 events rapidly
    var accepted: u64 = 0;
    var dropped: u64 = 0;
    var i: u64 = 0;
    while (i < 1000) : (i += 1) {
    ...
    const m = getMetrics();
    // Invariant must hold: accepted == popped + dropped + pending
    try std.testing.expect(m.verifyInvariant());

    // At most 16 per priority * 3 priorities = 48 accepted
    // Rest must be dropped
    try std.testing.expect(accepted <= 48);
    try std.testing.expect(dropped >= 952);

    // High watermark should have been set
    try std.testing.expect(m.high_watermark > 0);

    // Backpressure events should have fired (pressure went to high/saturated)
    try std.testing.expect(m.backpressure_events > 0);
}

test "Phase C: high watermark tracks max queue depth" {
    if (g_fabric_initialized) shutdownFabric(std.testing.allocator);
    try initFabric(std.testing.allocator, .{ .capacity_per_priority = 32 });
    defer shutdownFabric(std.testing.allocator);

    // Submit 20 events (below capacity of 32)
    var i: u64 = 0;
    while (i < 20) : (i += 1) {
        const event = canonical.create(.zig_core);
        _ = submitEvent(event);
    ...
    const m1 = getMetrics();
    try std.testing.expect(m1.high_watermark >= 20);

    // Pop all
    while (popEvent() != null) {}

    // High watermark should NOT decrease after popping
    const m2 = getMetrics();
    try std.testing.expect(m2.high_watermark >= 20);
    try std.testing.expect(m2.pending == 0);
}

test "Phase C: backpressure events counted" {
    if (g_fabric_initialized) shutdownFabric(std.testing.allocator);
    try initFabric(std.testing.allocator, .{ .capacity_per_priority = 4 });
    defer shutdownFabric(std.testing.allocator);
    ...
    const m = getMetrics();
    // At 100% capacity, pressure = saturated, so backpressure_events should be > 0
    try std.testing.expect(m.backpressure_events > 0);
    try std.testing.expect(m.pressure == .saturated);
}

test "Phase C: latency helpers return milliseconds" {
    if (g_fabric_initialized) shutdownFabric(std.testing.allocator);
    try initFabric(std.testing.allocator, .{ .capacity_per_priority = 16 });
    defer shutdownFabric(std.testing.allocator);
    ...
    const m = getMetrics();
    const last_ms = m.lastPopLatencyMs();
    const max_ms = m.maxPopLatencyMs();
    try std.testing.expect(last_ms >= 0.0);
    try std.testing.expect(max_ms >= 0.0);
}
