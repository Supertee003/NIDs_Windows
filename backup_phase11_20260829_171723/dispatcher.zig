//! runtime/dispatcher.zig - AEGIS Runtime Dispatcher (Rewrite Phase 11)
//!
...
pub fn processEvent(event: canonical.CanonicalEvent) void {
        ...
        if (evidence_list.count > 0) {
            ...
            if (g_aggregator) |*agg| {
                ...
                if (brain_int.isInitialized()) {
                    const advice = brain_int.advise(event, av, alerts, ti_match, flow_update_opt);
    ...
    // Phase 12+: policy_int.evaluateAndEnforce(event, av, alerts, ti_match, advice)
    // Phase 14+: forensics_int.logPipelineResult(event, av, alerts, ti_match, advice)
}
