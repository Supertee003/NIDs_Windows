//! runtime/dispatcher.zig - AEGIS Runtime Dispatcher (Rewrite Phase 12)
//!
...
pub fn processEvent(event: canonical.CanonicalEvent) void {
        ...
        if (evidence_list.count > 0) {
            ...
            if (g_aggregator) |*agg| {
                ...
                if (policy_int.isInitialized()) {
                    const decision = policy_int.evaluate(event, av, alerts, ti_match, advice);
    ...
    // Phase 13+: rust_pep.execute(decision)
    // Phase 14+: forensics_int.logPipelineResult(event, decision)
}
