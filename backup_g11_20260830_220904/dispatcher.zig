//! runtime/dispatcher.zig - AEGIS Runtime Dispatcher (Rewrite Phase 15)
//!
...
//!   (Replay Engine Phase 15 is a read-only analysis tool, not a pipeline stage)
//!
...
pub fn processEvent(event: canonical.CanonicalEvent) void {
        ...
        if (evidence_list.count > 0) {
            if (g_aggregator) |*agg| {
                ...
                if (forensics_int.isInitialized()) {
                    ...
                    if (seq > 0) {
                        std.log.debug("[FORENSICS] Logged event_id={d} seq={d}", .{ event.event_id, seq });
                    }
                }
            } else {
    ...
    // Pipeline complete - forensics recorded the full result.
}
