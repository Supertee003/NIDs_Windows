    ...
    while (true) {
        ...
        if (nose.hasEvents()) {
            if (nose.popEvent()) |event| {
                ...
                const result = policy_int.processEventFullPipeline(event, &g_detection_mgr, &.{});
                ...
                if (enrichment.matched) {
                    forensic_log.log(.{
                        .level = if (enrichment.enrichment.threat_category == .apt) .critical else .warn,
                ...
                if (det_ctx.matched) {
                    forensic_log.log(.{
                        .level = if (det_ctx.verdict == .match_block) .critical else .warn,
                        .event = if (det_ctx.verdict == .match_block) "DETECTION_BLOCK" else "DETECTION_ALERT",
                        .src_ip = det_ctx.event.source_ip,
                ...
                if (corr_result.incident_index != null) {
                    if (corr_result.linked_to_existing) {
                        forensic_log.log(.{
                            .level = if (corr_result.incident_severity >= 3) .critical else .warn,
                ...
                if (policy_result.decision != .allow) {
                    forensic_log.log(.{
                        .level = if (policy_result.decision == .block) .critical else .warn,
                        .event = "POLICY_DECISION",
                        .src_ip = policy_result.event.source_ip,
                        .session_id = policy_result.event.session_id,
                    });

                    if (policy_result.enforcement_result == .failed) {
                        forensic_log.log(.{
                            .level = .critical,
