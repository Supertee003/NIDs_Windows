    ...
    while (true) {
        ...
        if (connected or err == win.Win32Error.PIPE_CONNECTED) {
            ...
            if (read_success and bytes_read > 0) {
                ...
                // Named Pipe ไม่มี 5-tuple — ใช้ PacketContext ด้วย default values
                const ctx = nids_analyze.PacketContext{
                    .is_pipe = true,
                    .layer_id = 3,  // L2_PIPE
                };

                const is_safe = nids_analyze.inspect_packet(payload, ctx) catch |catch_err| blk: {
                    std.log.warn("[IPC SENSOR] Analyze error: {} - event allowed (fail-open)", .{catch_err});
