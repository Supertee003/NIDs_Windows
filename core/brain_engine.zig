...
pub const BrainEngine = struct {
    ...
    pub fn advise(
        ...
        // ThreatSeverity enum: none/low/medium/high/critical (no unknown/error).
        ...
        if (flow_update) |upd| {
            ...
            if (upd.flow.packet_count > 200) {
                signal_flow_anomaly += 30;
            } else if (upd.flow.packet_count > 100) {
                signal_flow_anomaly += 15;
            }

            // High byte count = suspicious
            if (upd.flow.byte_count > 50000) {
                signal_flow_anomaly += 20;
            } else if (upd.flow.byte_count > 10000) {
                signal_flow_anomaly += 10;
            }

            // Long duration = suspicious for established flows
            const duration_ns = upd.flow.last_seen_ns - upd.flow.start_ns;
            if (duration_ns > 60 * std.time.ns_per_s) {
                signal_flow_anomaly += 20;
            } else if (duration_ns > 10 * std.time.ns_per_s) {
                signal_flow_anomaly += 10;
            }
