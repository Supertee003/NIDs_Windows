...
fn ctrlHandler(ctrl_type: u32) callconv(.C) i32 {
    if (ctrl_type == 2) { // CTRL_C_EVENT
        g_shutdown_requested.store(true, .release);
        std.debug.print("\x1b[33m[SHUTDOWN] CTRL+C received -- draining connections...\x1b[0m\n", .{});
        return 1;
    }
    return 0;
}
...
pub fn reload_rules_atomic(allocator: std.mem.Allocator) !void {
    ...
    errdefer {
        for (temp_sig_list.items) |*sig| {
            ...
            std.log.info("[ANALYZE] Loaded {} rules into detection set", .{temp_sig_list.items.len});
    ...
    std.debug.print("\x1b[32m[ANALYZE] Loaded {d} secure rules\x1b[0m\n", .{valid_rule_count});
}

// =================================================================
// [ BRIDGE HELPERS Ã¢â‚¬â€ delegate to bridge_init ]
// =================================================================
...
pub fn inspect_packet(data: []const u8, ctx: PacketContext) !bool {
    ...
    if (!bridge_init.validatePayloadSafety(data.ptr, data.len)) return false;
...
fn handle_pipe_client(hPipe: win.HANDLE, allocator: std.mem.Allocator) void {
    ...
    std.log.info("[TCP] Connection established from {}", .{remote_addr});
...
fn pipe_listener(allocator: std.mem.Allocator) !void {
    ...
    if (ConvertStringSecurityDescriptorToSecurityDescriptorA(
        ...
        std.debug.print("[PIPE_LISTENER] WARNING: Failed to set pipe ACL\n", .{});
    }
    ...
    while (!g_shutdown_requested.load(.acquire)) {
        if (bridge_init.g_shutdown.load(.seq_cst)) break;
        const hPipe = CreateNamedPipeA(pipe_name, 3, 0, 255, 4096, 4096, 0, @ptrCast(&sec_attr));
        if (hPipe == win.INVALID_HANDLE_VALUE) {
            _ = g_analyze_errors.fetchAdd(1, .monotonic);
            std.debug.print("[PIPE_LISTENER] CreateNamedPipeA failed, retrying in 1s\n", .{});
        ...
        if (connected != 0 or @intFromEnum(err) == 535) {
            ...
            std.log.info("[TCP-LISTEN] Spawned handler thread", .{});
    ...
    log.info("Pipe listener shutting down", .{});
}
...
fn handle_tcp_client(stream: net.Stream, remote_addr: net.Address, allocator: std.mem.Allocator) void {
    ...
    var peak_buf: u64 = 0; // BP88: Peak buffer tracking
    if (packet_count > 0) {
        const _avg_pkt = bytes_read / packet_count;
        std.log.info("[TCP] Avg packet: {d} bytes", .{_avg_pkt});
    }
    if (peak_buf > 0) {
        const _buf_pct = @divTrunc(peak_buf * 100, 65535);
        std.log.info("[TCP] Buffer peak: {d}/65535 ({d}%)", .{peak_buf, _buf_pct});
    }
    if (packet_count == 0) {
        std.log.warn("[TCP] Zero-packet connection from {}", .{src_ip});
    }
    // BP81: Connection open log
    ...
    while (true) {
        if (bridge_init.g_shutdown.load(.seq_cst)) break;
        {
            ...
            if (_idle_ms > 60000) {
                std.log.warn("[TCP] Idle {}:{} for {d}ms", .{src_ip, src_port, _idle_ms});
            }
        }
        ...
        if (_bp92_ttfb_ns == 0) {
            ...
            std.log.info("[TCP] TTFB {d}ms from {}", .{_bp92_ttfb_ms, src_ip});
        }
        _bp94_last_act_ns = std.time.nanoTimestamp();
            std.debug.print("  [TCP] Read error, closing connection\n", .{});
            g_analyze_errors.fetchAdd(1, .seq_cst);
        };
        const elapsed_s: u64 = @intCast((std.time.nanoTimestamp() - start_ns) / std.time.ns_per_s);
        if (elapsed_s > 300) {
            std.debug.print("  [TCP] Max duration (300s) reached, closing\n", .{});
            break;
        }
        if (len > 65535) {
            std.debug.print("  [TCP] Oversized read: {d} bytes, closing\n", .{len});
            break;
        }
        }
            break;
        ...
        if (packet_count > 1_000_000) {
            std.debug.print("  [TCP] Max packets (1M) reached\n", .{});
            break;
        }
...
fn tcp_listener(allocator: std.mem.Allocator) !void {
    ...
    std.debug.print("[TCP] Listening on 0.0.0.0:12345\n", .{});
    defer server.deinit();
    ...
    const _drain_active = active_threads.load(.monotonic);
...
fn bridgeStatusReporter() void {
    ...
    if (bridge_init.g_shutdown.load(.seq_cst)) {
        ...
        const errs: u64 = @intCast(g_analyze_errors.load(.seq_cst));
        const err_per_min: u64 = if (up_s > 0) errs * 60 / up_s else 0;
        std.debug.print("  Error rate: ~{d} errors/min\n", .{err_per_min});
        const conn_per_min: u64 = if (up_s > 0) @intCast(g_total_connections.load(.monotonic)) * 60 / up_s else 0;
    ...
    const sh_conn: u64 = @intCast(g_total_connections.load(.monotonic));
    const sh_err: u64 = @intCast(g_analyze_errors.load(.monotonic));
...
reload_rules_atomic(allocator) catch |reload_err| {
    ...
    std.log.info("[ANALYZE] Analysis cycle started", .{});
    ...
    const t_status = std.Thread.spawn(.{}, bridgeStatusReporter, .{}) catch null;
    if (t_status) |t| t.detach();
    ...
    const t_tcp = std.Thread.spawn(.{}, tcp_listener, .{allocator}) catch |err| {
    std.debug.print("\x1b[33m[ANALYZE] Failed to spawn tcp_listener: {}\x1b[0m\n", .{err});
