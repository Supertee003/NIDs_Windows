// npcap_test_live.zig - AEGIS Phase 32 live capture verification tool (v3)
//
...
//
// v3 changes (after "only TCP, no ARP" field report):
//   - Duration mode: "30s" argument = capture for 30 SECONDS regardless of
//     packet count. Packet-count windows fill instantly with TCP on busy
//     networks, leaving no room for bursty ARP traffic.
//   - Live progress tick every 5s in duration mode (per-protocol counters)
//   - Zero-ARP diagnostic hint at end of run (how to force ARP + retest)
//   - Packet-count mode hard cap raised 30s -> 120s
//
// Usage:
//   npcap_test_live.exe                      - list devices only
//   npcap_test_live.exe auto                 - capture on the best adapter
//   npcap_test_live.exe <device_index|auto> [max_packets|Ns] ["bpf"]
//   Example: npcap_test_live.exe auto 30     - 30 packets (packet-count mode)
//   Example: npcap_test_live.exe auto 30s    - 30 seconds (duration mode)
//   Example: npcap_test_live.exe auto 30s arp            (ARP-only focus)
...
pub fn main() !void {
    ...
    if (args.len < 2) {
        std.debug.print("Usage: npcap_test_live.exe auto [max_packets|Ns] [\"bpf filter\"]\n", .{});
        std.debug.print("       npcap_test_live.exe <device_index> [max_packets|Ns] [\"bpf filter\"]\n", .{});
        std.debug.print("  '30'  = stop after 30 packets (hard time cap 120s)\n", .{});
        std.debug.print("  '30s' = stop after 30 seconds  (duration mode, unlimited packets)\n", .{});
        std.debug.print("Example: npcap_test_live.exe auto 30s arp     (see ARP traffic)\n", .{});
    ...
    // ---- Step 6: capture loop ----
    // args[2] "30"  -> packet-count mode (stop after N packets, 120s hard cap)
    // args[2] "30s" -> duration mode    (stop after N seconds, unlimited packets)
    var max_packets: usize = 20;
    var duration_secs: ?u64 = null;
    if (args.len >= 3) {
        const a = args[2];
        const is_duration = a.len > 1 and (a[a.len - 1] == 's' or a[a.len - 1] == 'S');
        if (is_duration) {
            if (std.fmt.parseInt(u64, a[0 .. a.len - 1], 10)) |secs| {
                duration_secs = secs;
                max_packets = 1_000_000;
            } else |_| {
                max_packets = std.fmt.parseInt(usize, a, 10) catch 20;
            }
        } else {
            max_packets = std.fmt.parseInt(usize, a, 10) catch 20;
        }
    ...
    const deadline_ms = start_ms + (if (duration_secs) |s|
        @as(i64, @intCast(s * 1000))
    else
        @as(i64, 120_000)); // packet-count mode: 120s hard cap (was 30s in v2)

    if (duration_secs) |s| {
        std.debug.print("\nCapturing for {d}s (duration mode, unlimited packets)...\n\n", .{s});
    } else {
        std.debug.print("\nCapturing up to {d} packets (120s timeout)...\n\n", .{max_packets});
    }
    ...
    while (printed < max_packets and std.time.milliTimestamp() < deadline_ms) {
        ...
        // Duration mode: live per-protocol progress tick every 5s so bursty
        // traffic (ARP / EAPOL / ICMP) is visible while the window is open.
        if (duration_secs != null) {
            const now_ms = std.time.milliTimestamp();
            if (now_ms - last_tick_ms >= 5_000) {
                last_tick_ms = now_ms;
                const t = sensor.getStats();
                std.debug.print("[t={d}s] tcp={d} udp={d} icmp={d} ipv6={d} arp={d} eapol={d} l2={d}+{d}\n", .{
                    @as(u64, @intCast(@divFloor(now_ms - start_ms, 1000))),
                    t.tcp_packets, t.udp_packets, t.icmp_packets, t.ipv6_packets,
                    t.arp_packets,  t.eapol_packets, t.l2_keepalives, t.l2_other_packets,
                });
            }
        }
    }
    ...
    std.debug.print("\n--- Capture statistics (elapsed {d}.{d:0>2}s) ---\n", .{
        @divFloor(elapsed_ms, 1000),
        @as(u64, @intCast(@mod(elapsed_ms, 1000))) / 10,
    });
    ...
    // ---- Step 8: zero-ARP diagnostic hint (v3) ----
    // ARP is BURSTY: a frame only appears when some host needs an IP->MAC
    // resolution (first contact / cache expiry). A packet-count window on a
    // busy TCP-heavy network can therefore legitimately end with arp=0 even
    // though ARP parsing works. Tell the user how to prove it end-to-end.
    if (stats.arp_packets == 0 and stats.packets_captured > 0) {
        std.debug.print("\n[INFO] No ARP frames observed (arp=0). This is a TRAFFIC condition,\n", .{});
        std.debug.print("       not a capture failure - ARP only appears when a host must\n", .{});
        std.debug.print("       resolve an IP -> MAC mapping. To force ARP frames and verify:\n", .{});
        std.debug.print("         1) in an ADMIN terminal, flush the ARP cache:  arp -d *\n", .{});
        std.debug.print("         2) find your gateway via 'ipconfig', then:     ping <gateway> -n 3\n", .{});
        std.debug.print("            (e.g. ping 192.168.1.1 -n 3 - forces request + reply)\n", .{});
        std.debug.print("         3) rerun with an ARP-focused time window:\n", .{});
        std.debug.print("              .\\npcap_test_live.exe auto 30s arp\n", .{});
        std.debug.print("       Lines like '#12 ARP request 192.168.1.x -> 192.168.1.1' and the\n", .{});
        std.debug.print("       'arp' counter in the stats block confirm end-to-end ARP capture.\n", .{});
    }
}
