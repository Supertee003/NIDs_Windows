...
// ============================================================
// Cross-platform socket handle + invalid sentinel
// ============================================================
// On Linux/macOS, posix.socket_t is c_int (i32), invalid socket = -1.
// On Windows, posix.socket_t is ws2_32.SOCKET (*opaque), invalid = INVALID_SOCKET.
// NOTE: posix.fd_t differs from posix.socket_t on Windows (HANDLE vs SOCKET),
// so the socket fields MUST use socket_t, not fd_t.
pub const SocketHandle = posix.socket_t;
pub const INVALID_FD: SocketHandle = if (builtin.os.tag == .windows)
...
pub const TcpTransport = struct {
    ...
    pub fn adoptFd(self: *TcpTransport, fd: SocketHandle) TcpError!void {
        self.fd = fd;
...
test "TcpTransport init defaults" {
    ...
    try std.testing.expectEqual(INVALID_FD, t.fd);
