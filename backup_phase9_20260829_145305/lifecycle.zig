//! lifecycle.zig - AEGIS Runtime Lifecycle (Rewrite Phase 8)
//!
//! Manages init/shutdown of all subsystems in correct order.
//! main() calls runtime.start() and runtime.shutdown() - nothing else.
//!
//! Phase 8 changes (on top of Phase 7):
//!   - Added VerdictAggregator init/shutdown (owned by dispatcher module).
//!   - Aggregator is initialized AFTER Detection (it consumes evidence).
...
test "lifecycle: flow_int, detection_int, and aggregator initialized after start" {
    if (fabric.isInitialized()) {
        shutdown();
    }
