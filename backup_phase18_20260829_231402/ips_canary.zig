...
pub const IpsCanary = struct {
    ...
    pub fn createCanaryEvent(self: *IpsCanary, test: CanaryTest) canonical.CanonicalEvent {
        ...
        var event = test.create_event(token);
    ...
    pub fn verifyCanary(
        ...
        // Check verdict
        if (result.aggregated_verdict != test.expected_verdict) {
            self.total_failed += 1;
            self.last_health = if (test.id < 2) .failed else .degraded;
            return .{
                .test_id = test.id,
                .test_name = test.name,
                .status = .failed,
        ...
        // Check action
        if (result.policy_action != test.expected_action) {
            self.total_failed += 1;
            self.last_health = if (test.id < 2) .failed else .degraded;
            return .{
                .test_id = test.id,
        ...
        // Check PEP status
        if (result.pep_status != test.expected_pep_status) {
            self.total_failed += 1;
            self.last_health = if (test.id < 2) .failed else .degraded;
            return .{
                .test_id = test.id,
