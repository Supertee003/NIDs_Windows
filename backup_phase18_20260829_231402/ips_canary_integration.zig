...
pub fn createCanaryEvent(test: canary.CanaryTest) canonical.CanonicalEvent {
    if (!g_initialized) {
        return canonical.create(.zig_core); // fallback
    }
    if (g_canary) |*c| {
        return c.createCanaryEvent(test);
    }
...
pub fn verifyCanary(
    ...
    if (g_canary) |*c| {
        const result = c.verifyCanary(test, record);
        g_total_canaries += 1;
        if (result.isPassed()) {
            g_total_passed += 1;
        } else {
...
test "ips canary integration: full lifecycle (init, create, verify, health, shutdown)" {
    ...
    const record = forensics_engine.ForensicRecord{
        .result = .{
            .event_id = canary_mod.CanaryToken{ .magic = canary_mod.CANARY_MAGIC, .test_id = 0, .sequence = 1, .reserved = 0 }.pack(),
            .timestamp_ns = 1000,
