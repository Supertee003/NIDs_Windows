"""
AEGIS NIDS — Command Control Center v7.0
=========================================
ศูนย์กลางควบคุมระบบ AEGIS NIDS ทั้งหมด — รวม v6 features + v3 monitoring

Features:
  - Real-time DEFCON + subsystem status in header (6 subsystems)
  - Mouth Control (Start/Stop/Rebuild) — Rust Mouth special handling
  - Launch/Stop/Restart subsystems (all or individual)
  - Rule management (CRUD + Hot Reload + 5-tuple)
  - Threat graph visualization
  - Bridge IPC status & round-trip test & throughput
  - Test suite (IPC stress, Npcap, Full verification, Attack simulation)
  - Monitoring (real-time dashboard, log tail, health check, IPC throughput)
  - Alert injection (5-phase attack scenario)
  - DAEMON integration (call aegis_daemon.py commands)
  - Log reset with backup
  - Exit with "Stop All?" confirmation
...
LOG_FILE = os.path.join("logs", "anomalous.json")
...
# ── Subsystem definitions (6 subsystems) ──
SUBSYSTEMS = [
    ...
    {"name": "NOSE",      "lang": "Go",     "exe": "aegis-nose.exe",           "py": "windows_perf.go"},
    {"name": "MOUTH",     "lang": "Rust",   "exe": "windows_sec_monitor.exe", "py": None},
    {"name": "DAEMON",    "lang": "Python", "exe": "python.exe",              "py": "aegis_daemon.py"},
]
...
def check_subsystem(sub):
    """ตรวจ subsystem กำลังทำงานหรือไม่ → (running, pid)"""
    exe = sub["exe"]
    py = sub.get("py")
...
def show_header():
    ...
    print(f"{'═' * 64}")
    print(f"  {C.BLD}{C.CYN}AEGIS NIDS — COMMAND CENTER (v7){C.RST}")
    print(f"  {defcon_str}")
    print(f"  Mouth: {mouth_str} | Active: {C.BLD}{running}/{total}{C.RST}  {dots}")
...
def menu_system():
  ...
  {C.BGRN}4{C.RST}  [STATUS]    แสดงสถานะ subsystem ทุกตัว
  {C.YEL}0{C.RST}  [BACK]
""")
        ...
        if choice == '1':
            ...
            if os.path.exists(bat):
                subprocess.Popen(["cmd", "/c", "start", "AEGIS Launcher", bat, "--no-dashboard"], shell=True)
                print(f"  {C.BGRN}OK{C.RST} run_aegis.bat launched")
            else:
                _launch_all_subsystems()
            input_pause()
            ...
            if os.path.exists(bat):
                result = subprocess.run(["cmd", "/c", bat, "--force"], capture_output=True, text=True, timeout=15)
                print(result.stdout)
            else:
            ...
            if os.path.exists(bat):
                subprocess.Popen(["cmd", "/c", "start", "AEGIS Launcher", bat, "--no-dashboard"], shell=True)
            print(f"  {C.BGRN}OK{C.RST} Restart initiated.")
            input_pause()

        elif choice == '4':
            _show_subsystem_detail()
            input_pause()
        elif choice == '0':
            break
...
def _launch_all_subsystems():
    cmds = [
        ("BRIDGE", ["build\\Release\\aegis_bridge.exe"], 0),
        ("CORE",   ["zig-out\\bin\\aegis-nids.exe"],     3),
        ("BRAIN",  [sys.executable, "windows_brain.py"],  6),
        ("NOSE",   ["aegis-nose.exe"],                     8),
        ("MOUTH",  ["windows_sec_monitor.exe"],           9),
    ]
    for name, cmd, delay in cmds:
        if delay > 0:
            time.sleep(2)
        exe = cmd[0] if len(cmd) == 1 else cmd[-1]
        if not os.path.exists(exe):
            print(f"  {C.YEL}SKIP{C.RST} {name}: {exe} not found")
            continue
        try:
            ...
            print(f"  {C.BGRN}OK{C.RST}   {name}: {' '.join(cmd)}")
...
# =====================================================================
# 2. MOUTH CONTROL (Rust special handling)
# =====================================================================
...
def menu_mouth():
    ...
    while True:
        ...
        # Check Mouth status
        mouth_sub = next(s for s in SUBSYSTEMS if s["name"] == "MOUTH")
  ...
  Status: {status}
  {'─' * 45}
  {C.BGRN}1{C.RST}  [START]     เปิด Mouth (windows_sec_monitor.exe)
...
""")
        ...
        if choice == '1':
            if running:
                ...
                print(f"\n  {C.CYN}[START]{C.RST} Starting Mouth...")
                if os.path.exists("windows_sec_monitor.exe"):
                    subprocess.Popen(["cmd", "/c", "start", "AEGIS MOUTH (Rust)",
                                      "windows_sec_monitor.exe"], shell=True)
                    print(f"  {C.BGRN}OK{C.RST} windows_sec_monitor.exe launched")
                else:
                    print(f"  {C.YEL}[!]{C.RST} EXE not found — try Rebuild first")
            input_pause()

        elif choice == '2':
            if not running:
                print(f"\n  {C.YEL}[!]{C.RST} Mouth not running")
            else:
                print(f"\n  {C.CYN}[STOP]{C.RST} Stopping Mouth...")
                subprocess.run(["taskkill", "/F", "/IM", "windows_sec_monitor.exe"], capture_output=True)
                print(f"  {C.BGRN}OK{C.RST} Mouth stopped")
            input_pause()
            ...
            rs_file = "windows_sec_monitor.rs"
            ...
            result = subprocess.run(
                ["rustc", "-O", rs_file, "-o", "windows_sec_monitor.exe"],
            ...
            if result.returncode == 0:
                ...
                subprocess.Popen(["cmd", "/c", "start", "AEGIS MOUTH (Rust)",
                                  "windows_sec_monitor.exe"], shell=True)
                print(f"  {C.BGRN}[OK]{C.RST} Mouth started")
            else:
            ...
            if result.returncode == 0:
                print(f"  {C.BGRN}[OK]{C.RST} cargo build succeeded (sec_monitor.dll)")
                # Note: cargo builds a library, not the standalone exe
                # Still need rustc for the exe
                print(f"  {C.DIM}Note: cargo builds sec_monitor.dll (FFI library){C.RST}")
...
""")
    ...
    if choice == '1':
        ...
        if os.path.exists(LOG_FILE) and os.path.getsize(LOG_FILE) > 0:
            ...
            backup = os.path.join("logs", f"anomalous_{ts}.json")
...
def menu_health():
  ...
  {C.BGRN}4{C.RST}  Real-time Dashboard (2s refresh)
  {C.YEL}0{C.RST}  Back
""")
        choice = input(f"  {C.BLD}Select (0-4): {C.RST}").strip()
...
def _show_health():
    ...
    try:
        ...
        print(f"\n  {C.BLD}Network:{C.RST}  ↑{net.bytes_sent/1024**2:.1f}MB  ↓{net.bytes_recv/1024**2:.1f}MB")
...
def _run_realtime_dashboard():
    ...
    try:
        while True:
            ...
            print(f"  {'═' * 58}")
            print(f"  {defcon_str}")
            print(f"  Subsystems: {C.BLD}{running}/6{C.RST} running\n")
            ...
            if PSUTIL_AVAILABLE:
                try:
                    net = psutil.net_io_counters()
                    print(f"\n  {C.DIM}NET  ↑{net.bytes_sent/1024**2:.1f}MB  ↓{net.bytes_recv/1024**2:.1f}MB{C.RST}")
                except Exception:
                    pass
...
# =====================================================================
# 7. STATUS (All subsystems)
# =====================================================================

def menu_status():
    """แสดงสถานะ subsystem ทุกตัว (same as SYSTEM → STATUS)"""
    clear_screen()
    show_header()
    _show_subsystem_detail()
...
def menu_tests():
  ...
  {'─' * 50}
  {C.BGRN}1{C.RST}  IPC Stress Test     throughput + multi-channel
  {C.BGRN}2{C.RST}  Npcap Capture Test  packet capture + IPC
  {C.BGRN}3{C.RST}  Full Verification   12 tests comprehensive
  {C.BGRN}4{C.RST}  Alert Integration   Brain→DEFCON→Mouth pipeline
...
""")
        ...
        if choice == '1':
            ...
            _run_test_script("test_e2e.py", "E2E Test")
        elif choice == '0':
            break
...
def menu_daemon():
  ...
  {'─' * 50}
  {C.BGRN}1{C.RST}  start       เปิด daemon (background mode)
  {C.BGRN}2{C.RST}  stop        หยุด daemon
  {C.BGRN}3{C.RST}  restart     รีสตาร์ท daemon
  {C.BGRN}4{C.RST}  status      สถานะ daemon
  {C.BGRN}5{C.RST}  health      ตรวจสุขภาพระบบ
  {C.BGRN}6{C.RST}  rules       Hot-reload rules
  {C.BGRN}7{C.RST}  logs        Tail logs (real-time)
  {C.YEL}0{C.RST}  Back
""")
        choice = input(f"  {C.BLD}Select (0-7): {C.RST}").strip()

        daemon_cmds = {
            '1': 'start', '2': 'stop', '3': 'restart',
            '4': 'status', '5': 'health', '6': 'rules', '7': 'logs',
        }
        ...
        if choice in daemon_cmds:
            ...
            try:
                if cmd == 'logs':
                    # Logs needs interactive handling
                    result = subprocess.run(
...
def main_menu():
  ...
  {'─' * 50}
  {C.BGRN}1{C.RST}  [RUN]      Launch ALL via run_aegis.bat
  {C.BGRN}2{C.RST}  [MOUTH]    Mouth Control (Start/Stop/Rebuild)
  {C.BGRN}3{C.RST}  [RULES]    Manage Detection Rules
  {C.BGRN}4{C.RST}  [LOGS]     Reset Anomalous Logs (with backup)
  {C.BGRN}5{C.RST}  [GRAPH]    Generate & View Threat Map
  {C.BGRN}6{C.RST}  [HEALTH]   System Health & Bridge Status
  {C.BGRN}7{C.RST}  [STATUS]   All Subsystem Status
  {C.BGRN}8{C.RST}  [TEST]     Run E2E / Test Suite
  {C.BGRN}9{C.RST}  [DAEMON]   Call aegis_daemon Commands
  {C.BLD}0{C.RST}  [EXIT]     Shutdown Console + Stop All?
""")

        if running == 6:
            print(f"  {C.BGRN}● AEGIS FULLY OPERATIONAL{C.RST}")
        elif running > 0:
            print(f"  {C.BYEL}◐ PARTIALLY RUNNING ({running}/6){C.RST}")
        else:
            print(f"  {C.BRED}○ SYSTEM STOPPED{C.RST}")

        choice = input(f"\n  {C.BLD}Select (0-9): {C.RST}").strip()
