...
# ── DLL search path setup (Python 3.8+ requires os.add_dll_directory) ──
import os as _os
if hasattr(_os, 'add_dll_directory'):
    _this_dir = _os.path.dirname(_os.path.abspath(__file__))
    _os.add_dll_directory(_this_dir)
    _build_dir = _os.path.join(_this_dir, 'build', 'Release')
    if _os.path.isdir(_build_dir):
        _os.add_dll_directory(_build_dir)
...
try:
    _shield = ctypes.CDLL("aegis_shield.dll")
    ...
    logger.warning("Rust shield library not available — forensic hashing disabled")
    SHIELD_AVAILABLE = False

# ─── ctypes Bridge to IPC DLL (aegis_ipc.dll) ───
try:
    # Try C wrapper first (unmangled C exports for Python ctypes)
    _ipc = ctypes.CDLL("aegis_ipc_c.dll")
    print("[Brain] IPC C wrapper loaded (unmangled exports)")
    # Set restype only on available functions
    if hasattr(_ipc, 'aegis_ipc_init'):
        _ipc.aegis_ipc_init.restype = ctypes.c_int32
    if hasattr(_ipc, 'aegis_ipc_get_stats'):
        _ipc.aegis_ipc_get_stats.restype = ctypes.c_int32
    IPC_AVAILABLE = True
except (OSError, AttributeError):
    logger.warning("IPC bridge DLL functions not available (C++ mangling) — stats unavailable")
    IPC_AVAILABLE = False
...
# --- Cython wrapper (faster than ctypes) ---
try:
    from aegis_shield_cy import AegisShield as _CythonShield
    _HAS_CYTHON = True
except ImportError:
    _CythonShield = None
    _HAS_CYTHON = False
