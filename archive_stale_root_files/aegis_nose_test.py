...
go_src = os.path.join(SCRIPT_DIR, "windows_perf.go")
...
try:
    r = subprocess.run(["tasklist", "/FI", "IMAGENAME eq aegis-nose.exe", "/NH"],
                       capture_output=True, text=True, timeout=5)
    if "aegis-nose" in r.stdout.lower():
        nose_running = True
...
exe_path = os.path.join(SCRIPT_DIR, "aegis-nose.exe")
