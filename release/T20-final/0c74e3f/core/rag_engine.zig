...
pub const RagEngine = struct {
    ...
    pub fn loadBuiltinEntries(self: *RagEngine) void {
        const entries = [_]RagEntry{
            ...
            // Entry 8 removed: was wildcard (ip=0, rule_id=0) that matched ALL queries.
            // Wildcard entries cause false matches on "unknown" tests.
            // Future: add wildcard support with explicit flag, not ip=0.
        };
...
test "RagEngine loadBuiltinEntries adds entries" {
    ...
    try std.testing.expect(engine.knowledgeCount() == 7);
}
