...
pub fn checkRpoRto(
    ...
    // If no snapshot exists (last_snapshot == 0), RPO is always violated --
    // there is no recovery point at all, so the "time since last snapshot" is
    // conceptually infinite (we use a value larger than rpo_ms to force violation).
    const last_snapshot = manager.lastSnapshotMs();
    const has_no_snapshot = (last_snapshot == 0);
    const time_since_last_snapshot_ms = if (has_no_snapshot) (rpo_ms + 1) else (now_ms - last_snapshot);
    const rpo_satisfied = !has_no_snapshot and (time_since_last_snapshot_ms <= rpo_ms);
...
pub fn verifyRpo() RpoCheck {
    ...
    // With no snapshots, RPO is always violated (no recovery point exists).
    // time_since_last_snapshot_ms is set to rpo_ms + 1 (forces violation).
    const empty_manager = SnapshotManager.init();
    const status_empty = checkRpoRto(empty_manager, 50000, 0);
    const rpo_no_snapshot_violates = !status_empty.rpo_satisfied and
        status_empty.time_since_last_snapshot_ms == (DEFAULT_RPO_MS + 1);
