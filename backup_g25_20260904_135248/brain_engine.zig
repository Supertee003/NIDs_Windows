...
pub const BrainAdviceKind = enum(u8) {
    ...
    pub fn recommendsChange(self: BrainAdviceKind) bool {
        return self == .escalate or self == .de_escalate or self == .escalate_to_block;
    }
};
...
pub const BrainAdvisor = struct {
    ...
    pub fn advise(
        ...
        var kind: BrainAdviceKind = .keep_verdict;
...
test "BrainAdviceKind.recommendsChange classifies correctly" {
    ...
    try std.testing.expect(!BrainAdviceKind.keep_verdict.recommendsChange());
    try std.testing.expect(BrainAdviceKind.escalate.recommendsChange());
    try std.testing.expect(BrainAdviceKind.de_escalate.recommendsChange());
