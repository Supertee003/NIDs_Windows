...
pub fn main() !void {
    ...
    for (devices[0..count], 0..) |*dev, i| {
        ...
        std.debug.print("       [{d}] {s}\n", .{ i, dev.name() });
        std.debug.print("           desc=\"{s}\" ip={s} loopback={} up={}\n\n", .{
    ...
    if (args.len < 2) {
        std.debug.print("Usage: npcap_test_live.exe <device_index> [max_packets] [\"bpf filter\"]\n", .{});
        std.debug.print("Example: npcap_test_live.exe 0 50 \"tcp port 80\"\n", .{});
        return;
    }

    // ---- Step 3: resolve target device (index or name) ----
    const target = args[1];
