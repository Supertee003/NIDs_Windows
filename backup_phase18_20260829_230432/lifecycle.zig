//! lifecycle.zig - AEGIS Runtime Lifecycle (Rewrite Phase 17)
//!
//! Manages init/shutdown of all subsystems in correct order.
//! main() calls runtime.start() and runtime.shutdown() - nothing else.
//!
//! Phase 17 changes (on top of Phase 16):
//!   - Added Performance Harness to start()/shutdown() sequence.
//!   - Performance is a read-only measurement tool (not a pipeline stage).
