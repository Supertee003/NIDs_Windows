...
// =================================================================
// [ G4: DISPATCHER DECOMPOSITION — Stage helpers ]
// G4 requirement: split inspect_packet into helpers with input/output/fallback/metrics
// =================================================================

/// G4: Flow stage — track flow state for network events.
/// Input: PacketContext + data_len. Output: *FlowEntry. Fallback: N/A. Metrics: flow_table.count.
fn processFlowStage(ctx: PacketContext, data_len: usize) ?*FlowEntry {
    if (ctx.is_pipe) return null;
    return &flow_table.lookupOrCreate(ctx, data_len, std.time.milliTimestamp());
}

/// G4: Detection stage — Rust Tier-3 safety + Aho-Corasick Tier-1.
/// Input: data + ruleset. Output: ?*const SecureRule (matched rule or null).
/// Fallback: null = no match. Metrics: total_rejected.
fn processDetectionStage(data: []const u8, current_ruleset: *const SecureRuleSet) ?*const SecureRule {
    // Tier-3: Rust memory safety pre-screen (returns false = unsafe → reject)
    ...
    var curr: usize = 0;
    var curr_ruleset = current_ruleset;
    var final_matched_rule: ?*const SecureRule = null;

    for (data) |char| {
        const c = @as(usize, char);
        curr = curr_ruleset.ac_engine.nodes.items[curr].next[c];
        var temp = curr;
        while (temp != 0) {
            for (curr_ruleset.ac_engine.nodes.items[temp].matches.items) |idx| {
                const rule = &curr_ruleset.signatures[idx];
            ...
            temp = curr_ruleset.ac_engine.nodes.items[temp].fail;
        }
        if (final_matched_rule != null) break;
    }
    return final_matched_rule;
}

/// G4: Correlation stage — update AtomicThreatTracker based on match.
/// Input: rule + ctx. Output: ThreatState. Fallback: CLEAN (no-op).
/// Metrics: match_count.
fn processCorrelationStage(rule: ?*const SecureRule) ThreatState {
    if (rule == null) return .CLEAN;
    ...
    if (global_attacker_tracker.getMatchCount() >= 3) {
        if (global_attacker_tracker.step2_verifyThreat()) {
            std.debug.print("[G7] Threat VERIFIED — {d} matches\n", .{global_attacker_tracker.getMatchCount()});
        }
    }
    return global_attacker_tracker.getState();
}

/// G4: Intelligence stage — send alert to Brain via UDP.
/// Input: alert struct. Output: void. Fallback: UDP fails silently. Metrics: N/A.
fn processIntelligenceStage(allocator: std.mem.Allocator, alert: anytype) void {
    send_to_brain(allocator, alert) catch {};
}

/// G4: PEP stage — call Rust PEP if Block action.
/// Input: action string + ctx. Output: bool (true = allow, false = block).
/// Fallback: if PEP unavailable, fall back to direct block. Metrics: PEP stats.
fn processPEPStage(action: []const u8, ctx: PacketContext) bool {
    if (!std.mem.eql(u8, action, "Block")) return true;

    // G7: Mark threat as BLOCKED
    _ = global_attacker_tracker.step3_block();

    // G10: Call Rust PEP (if available)
    if (fn_pep_enforce_action) |pep_fn| {
        const decision = PepDecision{
            .action = 2, // Block
            .source_ip = ctx.source_ip,
            .rule_id = 0,
            .policy_version = 1,
            .event_id = @intCast(event_accounting.total_input.load(.monotonic)),
            .confidence = 90,
        };
        const result = pep_fn(&decision);
        if (!result.success) {
            std.debug.print("[PEP] Enforcement failed (error={d})\n", .{result.error_code});
        }
    }

    std.debug.print("\x1b[31;1m[ AEGIS CORE ] !!! BLOCK !!! Connection Terminated\x1b[0m\n", .{});
    return false;
}

/// G4: Forensic stage — write forensic record.
/// Input: rule + ctx + event_id. Output: void. Fallback: log error. Metrics: forensic count.
fn processForensicStage(rule: *const SecureRule, ctx: PacketContext, event_id: u64) void {
    // G11: Write forensic record to logs/anomalous.json
    var buf: [512]u8 = undefined;
    const record = std.fmt.bufPrint(&buf,
        "{{\"forensic_id\":{d},\"event_id\":{d},\"rule\":\"{s}\",\"severity\":\"{s}\",\"action\":\"{s}\",\"src_ip\":{d},\"dst_ip\":{d},\"timestamp\":{d}}}\n",
        .{ event_id, event_id, rule.name, rule.severity, rule.action, ctx.source_ip, ctx.dest_ip, std.time.timestamp() }
    ) catch return;

    // Append to forensic log
    const log_path = "logs/anomalous.json";
    var file = std.fs.cwd().createFile(log_path, .{ .truncate = false }) catch return;
    defer file.close();
    file.seekFromEnd(0) catch return;
    file.writeAll(record) catch return;
}

// =================================================================
// [ G5: EVENT FATE — fate enum + trace ]
// G5 requirement: ACCEPTED/PROCESSED/REJECTED/FABRIC_DROPPED/EXPIRED/FAILED/ARCHIVED
// invariant: input = accepted + rejected + dropped + expired + failed
// =================================================================
...
pub const EventTrace = struct {
    ...
    stage: []const u8 = "",
    timestamp_ms: i64 = 0,
    fate: EventFate = .accepted,
    reason: [64]u8 = [_]u8{0} ** 64,
    reason_len: u8 = 0,

    pub fn setReason(self: *EventTrace, r: []const u8) void {
        const n = @min(r.len, 64);
        @memcpy(self.reason[0..n], r[0..n]);
        self.reason_len = @intCast(n);
    }
    pub fn reasonStr(self: *const EventTrace) []const u8 {
        return self.reason[0..self.reason_len];
    }
};

// =================================================================
// [ G7: CORRELATION — Incident + Evidence ]
// G7 requirement: incident_id, entity relationships, evidence set
// =================================================================

pub const Evidence = struct {
    event_id: u64 = 0,
    rule_id: u32 = 0,
    rule_name: [64]u8 = [_]u8{0} ** 64,
    rule_name_len: u8 = 0,
    tier: u8 = 0, // 1=Aho-Corasick, 2=regex, 3=Rust
    confidence: u8 = 0,
    flow_id: u64 = 0,
    timestamp_ms: i64 = 0,
};
...
pub const Incident = struct {
    ...
    created_ms: i64 = 0,
    source_ip: u32 = 0,
    threat_state: ThreatState = .CLEAN,
    evidence: [8]Evidence = [_]Evidence{.{}} ** 8,
    evidence_count: u8 = 0,
    match_count: u32 = 0,

    pub fn addEvidence(self: *Incident, ev: Evidence) void {
        if (self.evidence_count >= 8) return;
        self.evidence[self.evidence_count] = ev;
        self.evidence_count += 1;
    }
};

var global_incident: Incident = .{};
var next_incident_id: u64 = 1;

// =================================================================
// [ G8: INTELLIGENCE — fail-soft for Brain/RAG/TI ]
// G8 requirement: Brain=advisory, RAG=context, TI=evidence; fail-soft test
// =================================================================
...
pub const IntelligenceState = struct {
    ...
    pub fn failSoftCheck(self: *const IntelligenceState) void {
        if (!self.brain_available) {
            std.debug.print("[G8] Brain unavailable — detection continues without Tier-2\n", .{});
        }
        if (!self.rag_available) {
            std.debug.print("[G8] RAG unavailable — running without context enrichment\n", .{});
        }
        if (!self.ti_available) {
            std.debug.print("[G8] Threat Intel unavailable — running without IOC feeds\n", .{});
        }
    }
};

var intelligence_state: IntelligenceState = .{};

// =================================================================
// [ G9: POLICY PLANE — typed PolicyIR + key lifecycle ]
// G9 requirement: Policy Definition → Compiler → Canonical Policy IR →
//                 SHA-256 → Ed25519 Sign → Trust Verification → Rust PEP
// =================================================================

pub const PolicyValueType = enum(u8) {
    ipv4 = 0,
    ipv6 = 1,
    cidr = 2,
    string = 3,
...
pub const PolicyRule = struct {
    ...
    specificity: u32 = 0,
    version: u64 = 0,
    action: u8 = 0, // PepAction (0=allow, 1=alert, 2=block, 3=rate_limit, 4=quarantine)
    value_type: PolicyValueType = .string,
    value: [128]u8 = [_]u8{0} ** 128,
    value_len: u8 = 0,

    pub fn valueStr(self: *const PolicyRule) []const u8 {
...
pub const PolicyIR = struct {
    ...
    created_at_ms: i64 = 0,
    expires_at_ms: i64 = 0,
    signer: [64]u8 = [_]u8{0} ** 64,
    signer_len: u8 = 0,
    key_id: [32]u8 = [_]u8{0} ** 32,
    key_id_len: u8 = 0,
    sha256: [32]u8 = [_]u8{0} ** 32,
    signature: [64]u8 = [_]u8{0} ** 64,
    rules: [64]PolicyRule = [_]PolicyRule{.{}} ** 64,
    rule_count: u8 = 0,

    pub fn signerStr(self: *const PolicyIR) []const u8 {
        return self.signer[0..self.signer_len];
    }
    pub fn keyIdStr(self: *const PolicyIR) []const u8 {
        return self.key_id[0..self.key_id_len];
    }

    /// Deterministic conflict resolution: priority → specificity → rule_id → version
    pub fn resolveConflict(a: PolicyRule, b: PolicyRule) PolicyRule {
        if (a.priority != b.priority) return if (a.priority > b.priority) a else b;
        if (a.specificity != b.specificity) return if (a.specificity > b.specificity) a else b;
        if (a.rule_id != b.rule_id) return if (a.rule_id > b.rule_id) a else b;
        return if (a.version >= b.version) a else b;
    }

    /// Verify policy integrity: SHA-256 hash + Ed25519 signature
    pub fn verifyIntegrity(self: *const PolicyIR) bool {
        // Check version (must be > 0)
        if (self.version == 0) return false;
        // Check expiry
        const now = std.time.milliTimestamp();
        if (self.expires_at_ms > 0 and now > self.expires_at_ms) return false;
        // Check signer present
        if (self.signer_len == 0) return false;
        // Check key_id present
        if (self.key_id_len == 0) return false;
        // SHA-256 and Ed25519 verification would be done here using std.crypto
        // For now, return true if structural checks pass
        return true;
    }
};

pub const KeyLifecycleState = enum(u8) {
    provisioned = 0,
    active = 1,
    rotating = 2,
    revoked = 3,
...
pub const TrustStore = struct {
    key_id: [32]u8 = [_]u8{0} ** 32,
    key_id_len: u8 = 0,
    state: KeyLifecycleState = .provisioned,
    created_at_ms: i64 = 0,
    expires_at_ms: i64 = 0,
    // Ed25519 public key (32 bytes)
    public_key: [32]u8 = [_]u8{0} ** 32,

    pub fn isUsable(self: *const TrustStore) bool {
...
// Persistent rollback floor: minimum policy version that cannot be rolled back below
var persistent_rollback_floor: u64 = 0;

pub fn setRollbackFloor(version: u64) void {
    if (version > persistent_rollback_floor) {
        persistent_rollback_floor = version;
    }
}

pub fn checkRollback(version: u64) bool {
    return version >= persistent_rollback_floor;
}

// =================================================================
// [ G10: PEP FFI — PepDecision with request_id ]
// G10 requirement: every action has request_id/event_id/policy_id/policy_version/source/timestamp/result
// =================================================================

pub const PepDecision = extern struct {
    action: u8 = 0,
    source_ip: u32 = 0,
    rule_id: u32 = 0,
    ...
    confidence: u8 = 0,
    request_id: u64 = 0,
    source_operator: u8 = 0, // 0=auto, 1=cli, 2=federation
    timestamp_ms: i64 = 0,
};

pub const PepResult = extern struct {
    success: bool = false,
    action_taken: u8 = 0,
    audit_logged: bool = false,
    error_code: u32 = 0,
    result: u8 = 0, // 0=accepted, 1=rejected, 2=deferred, 3=failed, 4=no_op
};

const FnPepEnforceAction = *const fn (*const PepDecision) callconv(.c) PepResult;
var fn_pep_enforce_action: ?FnPepEnforceAction = null;

// =================================================================
// [ G11: FORENSICS — ForensicRecord + Replay engine ]
// G11 requirement: Action → PEP → Policy → Verdict → Evidence → Correlation → Event → Source
// =================================================================
...
pub const ForensicRecord = struct {
    ...
    event_id: u64 = 0,
    timestamp_ms: i64 = 0,
    decision_authority: [32]u8 = [_]u8{0} ** 32,
    decision_authority_len: u8 = 0,
    policy_version: u64 = 0,
    rule_id: u32 = 0,
    rule_name: [64]u8 = [_]u8{0} ** 64,
    rule_name_len: u8 = 0,
    ...
    dest_ip: u32 = 0,
    pep_action: u8 = 0,
    pep_success: bool = false,
    pep_error_code: u32 = 0,
    rollback_info: u8 = 0, // 0=no rollback, 1=can rollback
};

pub const ReplayRequest = struct {
    event_id: u64 = 0,
    ruleset_version: u64 = 0,
    policy_version: u64 = 0,
    brain_context_hash: [32]u8 = [_]u8{0} ** 32,
};

pub const ReplayResult = struct {
    original_action: u8 = 0,
    replayed_action: u8 = 0,
    matches: bool = false,
    timestamp_ms: i64 = 0,
};

/// G11: Replay engine — load historical event, re-run with specified version
pub fn replayEvent(req: ReplayRequest) ReplayResult {
    // Load event from logs/anomalous.json by event_id
    // Re-run through detection engine with specified ruleset version
    // Compare result with original enforcement
    // This is a stub that returns match=true (full implementation requires
    // structured event store + version-specified ruleset loading)
    _ = req;
    return .{
        .original_action = 2, // Block
        .replayed_action = 2, // Block
        .matches = true,
...
// --- [ 3-TIER FAST THREAT ANALYSIS ENGINE (G4: Decomposed Dispatcher) ] ---
// inspect_packet is the canonical dispatcher entry point.
// G4: Each stage is delegated to a helper with input/output/fallback/metrics.
...
pub fn inspect_packet(data: []const u8, ctx: PacketContext) !bool {
    ...
    // G4 Stage 1: Flow tracking (G6)
    _ = processFlowStage(ctx, data.len);

    // G8: Fail-soft check for intelligence layers
    intelligence_state.failSoftCheck();

    // G4 Stage 2: Detection (Rust Tier-3 + Aho-Corasick Tier-1)
    ...
    const matched_rule = processDetectionStage(data, current_ruleset);

    if (matched_rule) |rule| {
        // G4 Stage 3: Correlation (G7 AtomicThreatTracker + Incident)
        const threat_state = processCorrelationStage(matched_rule);

        // G7: Create/update incident
        if (global_incident.incident_id == 0) {
            global_incident = .{
        ...
        global_incident.match_count = global_attacker_tracker.getMatchCount();
        global_incident.addEvidence(.{
            .event_id = event_accounting.total_input.load(.monotonic),
            .rule_id = rule.crc32,
            .tier = 1,
            .confidence = 80,
            .flow_id = if (!ctx.is_pipe) flow_table.lookupOrCreate(ctx, data.len, 0).flow_id else 0,
            .timestamp_ms = std.time.milliTimestamp(),
        });
        ...
        // G4 Stage 4: Intelligence (send to Brain)
        processIntelligenceStage(allocator, alert);
        ...
        // G4 Stage 5: PEP enforcement (if Block)
        const allow = processPEPStage(rule.action, ctx);
        if (!allow) {
            // G4 Stage 6: Forensic record (G11)
            processForensicStage(rule, ctx, event_accounting.total_input.load(.monotonic));
        ...
        // G4 Stage 4: Intelligence (forward to Brain)
        processIntelligenceStage(allocator, forward_msg);
