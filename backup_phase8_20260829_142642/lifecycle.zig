//! lifecycle.zig - AEGIS Runtime Lifecycle (Rewrite Phase 7)
//!
//! Manages init/shutdown of all subsystems in correct order.
//! main() calls runtime.start() and runtime.shutdown() - nothing else.
//!
//! Phase 7 changes (on top of Phase 6):
//!   - Added Detection Engine to start()/shutdown() sequence.
//!   - Detection is initialized AFTER Flow (it needs FlowUpdate as input).
//!   - Detection is shutdown BEFORE Flow (reverse order).
...
test "lifecycle: flow_int and detection_int initialized after start" {
    if (fabric.isInitialized()) {
        shutdown();
    }
