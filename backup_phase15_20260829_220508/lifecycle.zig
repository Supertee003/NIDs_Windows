//! lifecycle.zig - AEGIS Runtime Lifecycle (Rewrite Phase 15)
//!
//! Manages init/shutdown of all subsystems in correct order.
//! main() calls runtime.start() and runtime.shutdown() - nothing else.
//!
//! Phase 15 changes (on top of Phase 14):
//!   - Added Replay Engine to start()/shutdown() sequence.
//!   - Replay is a read-only analysis tool (not a pipeline stage).
