...
const AegisPktMeta = extern struct {
    size: u32,
    orig_len: u32,
    timestamp: u64,
    layer_id: u16,
...
extern "aegis_ipc" fn aegis_ipc_init() callconv(.c) i32;
extern "aegis_ipc" fn aegis_ipc_read_packet(
    out_meta: *AegisPktMeta,
    out_buf: [*]u8,
    buf_size: *u32,
) callconv(.c) i32;
extern "aegis_ipc" fn aegis_ipc_get_stats(out_packets: *u32, out_dropped: *u32) callconv(.c) i32;
extern "aegis_ipc" fn aegis_ipc_shutdown() callconv(.c) void;

// ─── C-ABI Imports from Packet Parser (aegis_packet_parser.dll) ───
extern "aegis_packet_parser" fn aegis_quick_classify(
    frame_data: [*]const u8,
    frame_len: u32,
    out_proto: *u8,
    out_src_port: *u16,
...
extern "aegis_shield" fn aegis_shield_submit_packet(
...
extern "aegis_shield" fn aegis_semi_nids_init() callconv(.c) i32;
extern "aegis_shield" fn aegis_semi_nids_evaluate(
    src_ip: u32,
    dst_ip: u32,
    src_port: u16,
    dst_port: u16,
    ip_proto: u8,        // Must match Rust: u8 (not u16)
    threat_score: f64,
    confidence: u8,       // 0-4 matching Confidence enum
    risk_flags: u32,
    process_id: u32,
) callconv(.c) u8;        // Returns SemiNidsDecision as u8
extern "aegis_shield" fn aegis_semi_nids_set_policy(
    alert_id: u64,
    decision: u8,         // HumanDecision enum value
) callconv(.c) i32;
extern "aegis_shield" fn aegis_semi_nids_get_pending_count() callconv(.c) u32;
extern "aegis_shield" fn aegis_semi_nids_fail_open_status(
    out_active: *bool,
    out_cpu_pct: *u8,
    out_queue_pct: *u8,
) callconv(.c) u8;   // Returns LoadState (0-3), not i32
extern "aegis_shield" fn aegis_semi_nids_update_load(
    cpu_pct: u8,
    queue_pct: u8,
    pps: u64,
) callconv(.c) void;
extern "aegis_shield" fn aegis_semi_nids_block_ip(ip: u32, reason: u32) callconv(.c) i32;
extern "aegis_shield" fn aegis_semi_nids_unblock_ip(ip: u32) callconv(.c) i32;
// SemiNidsStats struct — must match Rust SemiNidsStats #[repr(C)] layout exactly
const SemiNidsStats = extern struct {
    total_evaluated: u64,
    total_passed: u64,
    total_alerted: u64,
    total_blocked: u64,
...
extern "aegis_shield" fn aegis_semi_nids_get_stats(
    out_stats: *SemiNidsStats,
) callconv(.c) i32;
extern "aegis_shield" fn aegis_semi_nids_maintenance() callconv(.c) u32;  // Returns expired temp block count
extern "aegis_shield" fn aegis_semi_nids_shutdown() callconv(.c) void;

// ─── C-ABI Import from Rust Correlation Engine (aegis_shield) ───
extern "aegis_shield" fn aegis_correlation_init() callconv(.c) i32;
...
const AhoCorasick = struct {
    goto_fn: [AHOCORASICK_MAX_STATES][AHOCORASICK_ALPHABET]u16,
    failure: [AHOCORASICK_MAX_STATES]u16,
    output: [AHOCORASICK_MAX_STATES]std.DynamicBitSet,
    state_count: u16,
    ...
    terminal_pattern: [AHOCORASICK_MAX_STATES]u16,
    ...
    fn init(allocator: std.mem.Allocator) !Self {
        var ac = Self{
            ...
            .pattern_count = 0,
            .terminal_pattern = undefined,
            .allocator = allocator,
        };

        // Initialize goto function: all transitions from root → root (self-loop)
        @memset(&ac.goto_fn[0], 0);
        @memset(&ac.failure, 0);
        @memset(&ac.terminal_pattern, 0);
    ...
    fn addPattern(self: *Self, pattern: []const u8) !u16 {
    ...
    fn scan(self: *const Self, payload: []const u8, out_matches: []u32) u32 {
        var state: u16 = 0;
...
// ─── Packet Batch Buffer ───
const PacketBatch = struct {
    metas: [CAPTURE_BATCH_SIZE]AegisPktMeta,
    payloads: [CAPTURE_BATCH_SIZE][MAX_PAYLOAD_SIZE]u8,
    payload_lens: [CAPTURE_BATCH_SIZE]u32,
    count: usize,
...
const CaptureEngine = struct {
    ...
    fn init(allocator: std.mem.Allocator, patterns: []const []const u8) !Self {
    ...
    fn processBatch(self: *Self) !void {
        ...
        for (0..self.batch.count) |i| {
            ...
            if (match_count > 0) {
                ...
                const decision_raw = aegis_semi_nids_evaluate(
                    meta.src_ip,
                    meta.dst_ip,
                    meta.src_port,
                    meta.dst_port,
                ...
                std.log.debug("Semi-NIDS: {} → {} (score={d:.0}, conf={}, flags=0x{x})", .{
    ...
    fn run(self: *Self) !void {
        ...
        std.log.info("AEGIS Capture: Stopped — {} packets, {} alerts", .{
...
pub fn init(allocator: std.mem.Allocator, patterns: []const []const u8) !void {
    ...
    const ipc_rc = aegis_ipc_init();
    if (ipc_rc != 0) return error.IPCInitFailed;

    // Initialize Rust Semi-NIDS engine (adaptive drop + fail-open + human-in-loop)
    const semi_rc = aegis_semi_nids_init();
    if (semi_rc != 0) return error.SemiNidsInitFailed;

    // Initialize Rust Correlation engine (cross-vector scoring)
    const corr_rc = aegis_correlation_init();
    if (corr_rc != 0) return error.CorrelationInitFailed;
...
pub fn getStats() struct { packets: u64, alerts: u64, blocked: u64, pending: u64, fail_open: u64 } {
    ...
    var pending: u64 = 0;
    var fail_open: u64 = 0;
    _ = aegis_semi_nids_get_stats(&total, &blocked, &alerts, &pending, &fail_open);
