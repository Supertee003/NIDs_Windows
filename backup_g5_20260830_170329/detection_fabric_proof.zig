...
pub fn verifyEvidenceAggregation(
    allocator: std.mem.Allocator,
) EvidenceAggregationCheck {
...
pub fn generateReport(allocator: std.mem.Allocator) G5Report {
    ...
    const aggregation = verifyEvidenceAggregation(allocator);
...
test "verifyEvidenceAggregation produces verdict" {
    const check = verifyEvidenceAggregation(std.testing.allocator);
    try std.testing.expect(check.isPassed());
